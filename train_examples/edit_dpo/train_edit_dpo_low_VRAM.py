...
def train(global_step=0):
    ...
    for epoch in range(first_epoch, args.num_train_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # --- VAE encoding: move to GPU, encode sequentially, then offload back ---
            vae.to(accelerator.device)
            with torch.no_grad():
                latents_win = vae.encode(images_win.to(weight_dtype).to(accelerator.device)).latent_dist.sample()
                latents_lose = vae.encode(images_lose.to(weight_dtype).to(accelerator.device)).latent_dist.sample()
                latents = torch.cat([latents_win, latents_lose], dim=0)
                latents = latents.to(dtype=weight_dtype)
                latents = (latents - vae.config.shift_factor) * vae.config.scaling_factor
                del latents_win, latents_lose

                ref_image_single = batch['ref_images']
                ref_latents_single = vae.encode(ref_image_single.to(weight_dtype).to(accelerator.device)).latent_dist.sample()
                ref_latents = torch.cat([ref_latents_single, ref_latents_single], dim=0)
                ref_latents = ref_latents.to(dtype=weight_dtype)
                ref_latents = (ref_latents - vae.config.shift_factor) * vae.config.scaling_factor
                del ref_latents_single, ref_image_single
            vae.to("cpu")
            torch.cuda.empty_cache()

            # --- Text encoding: move to GPU, encode, then offload back ---
            ...
            text_encoder.to("cpu")
            del text_output, text_input_ids, text_attention_mask, pixel_values, image_grid_thw
            torch.cuda.empty_cache()
            ...
            # Keep image shape for logging (reconstructed from latents)
            image_shape = (latents.shape[0], 3, latents.shape[2] * 8, latents.shape[3] * 8)
            ...
            with accelerator.accumulate(transformer):
                ...
                # --- Ref model: move to GPU, forward, then offload back ---
                ...
                ref_model.to("cpu")
                del ref_pred, ref_losses, ref_loss_win, ref_loss_lose
                torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    ref_model.to("cpu", weight_dtype)  # keep ref_model on CPU, move to GPU only when needed
    ...
    # Keep VAE and text_encoder on CPU; move to GPU only during encoding to save VRAM
    vae = AutoencoderKL.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="vae",  torch_dtype=weight_dtype).eval()

    text_encoder = AutoModel.from_pretrained(
        args.pretrained_model_name_or_path, subfolder="text_encoder" , torch_dtype=weight_dtype, trust_remote_code=True).eval()
